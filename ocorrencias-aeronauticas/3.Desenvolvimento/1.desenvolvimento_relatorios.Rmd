...
```{r define_alias_package, message=FALSE, error=TRUE, results='hide'}
######## Pacotes básicos
...
p_ocor_classif <- calcula_tab_frequencia_dplyr(
    ...
    specific_order = bs::c("ACIDENTE","INCIDENTE GRAVE", "INCIDENTE")
)
...
p_incidente <- p_ocor_classif %>%
    dp::filter(stg::str_detect(`Category`, "ACIDENTE") |
                  stg::str_detect(`Category`, "INCIDENTE GRAVE")) %>% 
    dp::pull(`Percentage`) %>% 
    bs::sum() %>% 
    bs::round(1) %>%
        mg::subtract(100,.)
...
p_ocor_ano[["Category"]] <- bs::as.factor(p_ocor_ano[["Category"]])
...
ggplot2_ocor_ano_classif <- gg::ggplot(
    ...
    gg::scale_x_continuous(
        name="Ano",
        labels = bs::as.character(bs::unique(p_ocor_ano_classif$ocorrencia_ano)),
        breaks = bs::unique(p_ocor_ano_classif$ocorrencia_ano)
    )
...
p_nao_incidente_ano <- p_ocor_ano_classif %>% dp::filter(`ocorrencia_classificacao` == "INCIDENTE") %>% 
    dp::pull(`Percentage`)
bs::names(p_nao_incidente_ano) <- bs::unique(p_ocor_ano_classif[["ocorrencia_ano"]])
...
p_ocor_uf[,'Category'] <- bs::as.character(p_ocor_uf[,'Category'],2)
p_ocor_uf[,'Percentage'] <- bs::round(p_ocor_uf[,'Percentage'],2)
...
if (bs::file.exists(bs::paste0(endereco_graficos_interativos, "map.rds"))) {
...
hw::saveWidget(widget = plot, file = bs::paste0(endereco_imagens,"plot.html"))
if (kn::is_html_output()) {
    plot
} else {
    wb::webshot(url = bs::paste0(endereco_imagens,"plot.html"), 
                     file =bs::paste0(endereco_imagens,"plot.png"),
                     delay = 0.2,
                     zoom=1.5)
}
...
p_ocor_uf_classif$`Percentage` <- bs::round(p_ocor_uf_classif$`Percentage`,1)
...
p_ocor_saida_pista_classif$`Percentage` <- bs::round(p_ocor_saida_pista_classif$`Percentage`,1)
...
p_ocor_saida_pista_classif_aux <- p_ocor_saida_pista_classif %>%
    dp::filter(stg::str_detect(`ocorrencia_classificacao`, "ACIDENTE"))
...
p_ocor_ocorrencia_tipo_new$`Percentage` <- bs::round(p_ocor_ocorrencia_tipo_new$`Percentage`,1)
p_ocor_ocorrencia_tipo_new[['Category']] <- bs::factor(p_ocor_ocorrencia_tipo_new[['Category']],
                                                   levels=bs::rev(p_ocor_ocorrencia_tipo_new[['Category']]))
...
ggplot2_p_ocor_ocorrencia_tipo_new <- ggplot2_p_ocor_ocorrencia_tipo_new + 
                                        gg::scale_x_discrete(
                                        labels = function(x) {
                                              is_long <- bs::nchar(x) > 25
                                              x[is_long] <- bs::paste0(bs::substr(x[is_long], 1, 25), ".")
                                              x
                                            }
                                          ) +
...
p_ocor_ocorrencia_tipo_new_classif$`Percentage` <- bs::round(p_ocor_ocorrencia_tipo_new_classif$`Percentage`,1)
...
ggplot2_ocor_ocorrencia_tipo_new_classif1 <- grafico_tab_frequencia_bivariada2(
    ...
    gg::scale_x_discrete(
  labels = function(x) {
        is_long <- bs::nchar(x) > 20
        x[is_long] <- bs::paste0(bs::substr(x[is_long], 1, 20), ".")
        x
      }) +
    gg::ylab("N° de ocorrências aeronáuticas") +
    gg::xlab("Tipo de ocorr. aeron.")
...
ggplot2_ocor_ocorrencia_tipo_new_classif3 <- grafico_tab_frequencia_bivariada3(
    ...
    labels = function(k) {
        is_long <- bs::nchar(k) > 19
        k[is_long] <- bs::paste0(bs::substr(k[is_long], 1, 19), ".")
        k
    }
) +
    gg::coord_flip(
